...
PATH_ROOT = "./content"
PATH_INIT = f"{PATH_ROOT}/images_init"
PATH_OUT = f"{PATH_ROOT}/images_out"
PATH_BATCH = f"{PATH_ROOT}/{BATCH_NAME}"
PATH_MODELS = f"{PATH_ROOT}/models"

sys.path.append("./resize_right")
...
model_256_path = f"{PATH_MODELS}/256x256_diffusion_uncond.pt"
model_512_path = f"{PATH_MODELS}/512x512_diffusion_uncond_finetune_008100.pt"
model_secondary_path = f"{PATH_MODELS}/secondary_model_imagenet_2.pth"
...
if diffusion_model == "256x256_diffusion_uncond":
    if os.path.exists(model_256_path) and check_model_SHA:
        ...
        if hash == model_256_SHA:
            ...
            subprocess.run(["wget", "--continue", model_256_link, "-P", PATH_MODELS])
            model_256_downloaded = True
    elif (
    ...
    if os.path.exists(model_512_path) and check_model_SHA:
        ...
        with open(model_512_path, "rb") as f:
            ...
            if hash == model_512_SHA:
                ...
                subprocess.run(
                    ["wget", "--continue", model_512_link, "-P", PATH_MODELS]
                )
                model_512_downloaded = True
    elif (
        ...
        subprocess.run(["wget", "--continue", model_512_link, "-P", PATH_MODELS])
        model_512_downloaded = True
...
if use_secondary_model == True:
    if os.path.exists(model_secondary_path) and check_model_SHA:
        ...
        if hash == model_secondary_SHA:
            ...
            subprocess.run(
                ["wget", "--continue", model_secondary_link, "-P", PATH_MODELS]
            )
            model_secondary_downloaded = True
    elif (
        ...
        subprocess.run(["wget", "--continue", model_secondary_link, "-P", PATH_MODELS])
        model_secondary_downloaded = True
...
##**Animation Settings:**
animation_mode = "None"  # ['None', '2D', 'Video Input']
# *For animation, you probably want to turn `cutn_batches` to 1 to make it quicker.*


# ####**Video Input Settings:**
video_init_path = "/content/training.mp4"  # {type: 'string'}
extract_nth_frame = 2  # {type:"number"}

if animation_mode == "Video Input":
    video_frames_folder = f"/content/video_frames"
    create_path(video_frames_folder)
    print(f"Exporting Video Frames (1 every {extract_nth_frame})...")
    try:
        subprocess.call(["rm", video_frames_folder, "/*.jpg"])
    except:
        print("")
...
if animation_mode == "Video Input":
    max_frames = len(glob(f"{video_frames_folder}/*.jpg"))

interp_spline = "Linear"  # Do not change, currently will not look good. param ['Linear','Quadratic','Cubic']{type:"string"}
angle = "0:(0)"  # {type:"string"}
zoom = "0: (1), 10: (1.05)"  # {type:"string"}
...
frames_scale = 1500  # {type: 'integer'}
...
if intermediate_saves and intermediates_in_subfolder is True:
    partialFolder = f"{PATH_BATCH}/partials"
    create_path(partialFolder)

    #  ---

#  ####**SuperRes Sharpening:**
#  *Sharpen each image using latent-diffusion. Does not run in animation mode. `keep_unsharp` will save both versions.*
sharpen_preset = "Off"  #  ['Off', 'Faster', 'Fast', 'Slow', 'Very Slow']
keep_unsharp = True  # {type: 'boolean'}

if sharpen_preset != "Off" and keep_unsharp is True:
    unsharpenFolder = f"{PATH_BATCH}/unsharpened"
    create_path(unsharpenFolder)
...
perlin_init = False  # {type: 'boolean'}
        ...
        if args.animation_mode == "2D":
            ...
            if frame_num > 0:
                ...
                if resume_run and frame_num == start_frame:
                    img_0 = cv2.imread(
                        PATH_BATCH + f"/{batch_name}({batchNum})_{start_frame-1:04}.png"
                    )
                else:
        ...
        for i in range(args.n_batches):
                            ...
                            if args.steps_per_checkpoint is not None:
                                if j % args.steps_per_checkpoint == 0 and j > 0:
                                    if args.intermediates_in_subfolder is True:
                                        ...
                                        image.save(f"{PATH_BATCH}/{filename}")
                            else:
            ...
            with image_display:
                if args.sharpen_preset != "Off" and animation_mode == "None":
                    ...
                    do_superres(imgToSharpen, f"{PATH_BATCH}/{filename}")
                    display.clear_output()
...
def save_settings():
    ...
    with open(
        f"{PATH_BATCH}/{batch_name}({batchNum})_settings.txt", "w+"
    ) as f:  # save settings
...
def download_models(mode):
    ...
    if mode == "superresolution":
        ...
        url_conf = "https://heibox.uni-heidelberg.de/f/31a76b13ea27482981b4/"
        url_ckpt = "https://heibox.uni-heidelberg.de/f/578df07c8fc04ffbadf3/"

        path_conf = f"{PATH_MODELS}/superres/project.yaml"
        path_ckpt = f"{PATH_MODELS}/superres/last.ckpt"
...
if secondary_model_ver == 2:
    ...
    secondary_model.load_state_dict(
        torch.load(f"{PATH_MODELS}/secondary_model_imagenet_2.pth", map_location="cpu")
    )
...
if __name__ == "__main__":
    paths = [PATH_INIT, PATH_OUT, PATH_BATCH, PATH_MODELS]
    ...
    n_batches = 1
    batch_size = 1
    resume_run = False  # {type: 'boolean'}
    run_to_resume = "latest"  # {type: 'string'}
    resume_from_frame = "latest"  # {type: 'string'}
    retain_overwritten_frames = False  # {type: 'boolean'}
    if retain_overwritten_frames is True:
        retain_folder = f"{PATH_BATCH}/retained"
        create_path(retain_folder)
    ...
    if resume_run:
        if run_to_resume == "latest":
            try:
                ...
                batchNum = len(glob(f"{PATH_BATCH}/{batch_name}(*)_settings.txt")) - 1
        else:
            batchNum = int(run_to_resume)
        if resume_from_frame == "latest":
            start_frame = len(glob(PATH_BATCH + f"/{batch_name}({batchNum})_*.png"))
        else:
            start_frame = int(resume_from_frame) + 1
            if retain_overwritten_frames is True:
                existing_frames = len(
                    glob(PATH_BATCH + f"/{batch_name}({batchNum})_*.png")
                )
                frames_to_save = existing_frames - start_frame
                print(f"Moving {frames_to_save} frames to the Retained folder")
                move_files(start_frame, existing_frames, PATH_BATCH, retain_folder)
    else:
        start_frame = 0
        batchNum = len(glob(PATH_BATCH + "/*.txt"))
        while (
            path.isfile(f"{PATH_BATCH}/{batch_name}({batchNum})_settings.txt") is True
            or path.isfile(f"{PATH_BATCH}/{batch_name}-{batchNum}_settings.txt") is True
        ):
            batchNum += 1
    ...
    model.load_state_dict(
        torch.load(f"{PATH_MODELS}/{diffusion_model}.pt", map_location="cpu")
    )
...
if skip_video_for_run_all == False:
    ...
    last_frame = final_frame  #  {type:"number"} You can change i to the number of the last frame you want to generate. It will raise an error if that number of frames does not exist.
    fps = 12  #  {type:"number"}
    view_video_in_cell = False  #  {type: 'boolean'}

    frames = []
    # tqdm.write('Generating video...')

    if last_frame == "final_frame":
        last_frame = len(glob(PATH_BATCH + f"/{folder}({run})_*.png"))
        print(f"Total frames: {last_frame}")

    image_path = f"{PATH_OUT}/{folder}/{folder}({run})_%04d.png"
    filepath = f"{PATH_OUT}/{folder}/{folder}({run}).mp4"
    ...
    process = subprocess.Popen(
        cmd, cwd=f"{PATH_BATCH}", stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
